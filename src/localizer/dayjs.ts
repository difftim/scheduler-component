...
export default function (dayjsLib) {
  ...
  function merge(date, time) {
    ...
    const dt = dayjs(date).startOf('day').format('YYYY-MM-DD');
    if (dayjsLib.tz) {
      const tzName = dayjs().$x?.$timezone ?? dayjsLib.tz.guess();
      // We do it this way to avoid issues when timezone switching
      return dayjs(`${dt} ${tm}`, 'YYYY-MM-DD HH:mm:ss', tzName).toDate();
    }

    return dayjsLib(`${dt} ${tm}`, 'YYYY-MM-DD HH:mm:ss').toDate();
  }
